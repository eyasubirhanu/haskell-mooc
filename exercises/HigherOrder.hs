...
product :: Num a => [a] -> a
product = go 1 --1 is starter acc and go is function in function
  where
    go :: Num a => a -> [a] -> a
    go acc []  = acc
...
isBST Empty = True
isBST (Bin l v r) = allValues (<= v) l && allValues (>= v) r && isBST l && isBST r
  where 
    allValues :: (a -> Bool) -> BinTree a -> Bool
    allValues _x Empty = True 
    allValues x (Bin l v r) = allValues x l  &&   x v  && allValues x r
...
-- https://stackoverflow.com/questions/52705485/what-does-the-in-keyword-do-in-haskell#:~:text=in%20goes%20along%20with%20let,expressions%20in%20a%20pure%20function.&text=would%20print%20%22hello%20world%22.&text=And%20you%20just%20need%20one,about%20anything%2C%20even%20functions%20themselves!
-- Task HigherOrder-12.
--
